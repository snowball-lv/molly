...
static char is_irq_spurious(uint32_t num) {
	...
	if (num == PIC_IRQ_BASE + PIC_IRQ_SPURIOUS_MASTER) {
		...
		if (pic_isr(PIC_MASTER) & PIC_IRQ_SPURIOUS_MASTER) {
			//legit
	...
	} else if (num == PIC_IRQ_BASE + PIC_IRQ_SPURIOUS_SLAVE) {
	
		if (pic_isr(PIC_SLAVE) & (PIC_IRQ_SPURIOUS_SLAVE >> 8)) {
			//legit
...
isr_w_fptr _ISR_WRAPPERS[MAX_INTERRUPTS] = {
	ISR_WRAPPER_NAME(0),
	ISR_WRAPPER_NAME(1),
	ISR_WRAPPER_NAME(2),
	ISR_WRAPPER_NAME(3),
